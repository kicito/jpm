...
export const ERR_JPM_EXISTS: Error = new Error(
  `${chalk.red('ERR_JPM_EXISTS')} jpm configuration is presented in package.json.`
)

export const ERR_NOT_JPM_PACKAGE: Error = new Error(
  `${chalk.red('ERR_NOT_JPM_PACKAGE')} Detected non jpm package, use ${chalk.bold('$ jpm init')} to initialize jpm package in current directory.`
)

export const ERR_TARGET_NOT_JPM_PACKAGE: Error = new Error(
  `${chalk.red('ERR_TARGET_NOT_JPM_PACKAGE')} Target dependency is not jpm package, please use node package manager to add the dependency.`
)

export const ERR_NOT_JPM: Error = new Error(
  `${chalk.red('ERR_NOT_JPM')} You must use ${chalk.bold('jpm')} to install this package.`
)

export const ERR_MVN_NOT_FOUND: Error = new Error(
  `${chalk.red('ERR_MVN_NOT_FOUND')} Unable to locate ${chalk.bold('mvn')} installation in system path`
)
