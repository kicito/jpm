...
export default class HooksPreInstall extends Command {
  ...
  Currently, it checks and throw error if the package manager is not ${chalk.bold(
    'jpm'
  )}
  eg. npm install JOLIE_PACKAGE should fail the installation`;

  static override examples = ['jpm hooks preinstall'];

  static override flags = {
    allowGlobal: Flags.boolean({
      description: `allow ${chalk.bold('npm install -g jolie-package')}`,
    }),
  };
  ...
  public async run(): Promise<void> {
    ...
    if (
      ...
      process.env['npm_execpath'].indexOf('jolie') === -1
    ) {
      throw ERR_NOT_JPM;
    }
  }
